...
export default function ChatsList({
  ...
  const acceptRequestDirectly = (req: ChatRequest) => {
    const sender = allUsers.find(u => u.id === req.senderId);
    if (!sender) return;

    // Create permanent chat room according to schema
    const newChat: Chat = {
      id: "chat_" + Date.now(),
                        ...
                        {(!currentUser.isAdult && isSenderAdult) && (
                          <div className="p-3.5 bg-red-950/20 border border-red-500/30 rounded-xl space-y-2" id="request-adult-warning-block">
                            ...
                            <p className="text-[11px] text-slate-300 leading-relaxed">
                              {currentUser.nickname}, Juan te ha enviado una solicitud de chat, tiene {sender?.age} años. Recuerda que es un adulto y debes de tener cuidado, te aconsejamos que salgas o que no aceptes.
                            </p>
                          </div>
                        )}
